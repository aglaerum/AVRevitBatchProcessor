...
def find_newest_revit():
    ...
    for year in search_years:
        ...
        if os.path.exists(revit_path):
            print("Found Revit {0} at {1}".format(year, revit_path))
            return revit_path
            break
    else:
        ...
        for root, dirs, files in os.walk(folder_path):
            for filename in files:
                if filename.endswith(file_ending):
                    fi.write(os.path.join(root, filename) + '\n')
...
def create_csv_file(rvt_file_list, pset_file_list, mapping_file_list, output_file):
    with open(output_file, 'w') as f:
        ...
        with open(rvt_file_list, 'r') as rvt_file_list, open(pset_file_list, 'r') as pset_file_list, open(mapping_file_list, 'r') as mapping_file_list:
