...
def GetSupportedRevitFiles(batchRvtConfig):
    ...
    if revitFileListData is not None:
        ...
        nonExistentRevitFileList = list(supportedRevitFileInfo for supportedRevitFileInfo in supportedRevitFileList if (not supportedRevitFileInfo.IsCloudModel() and not RevitFileExists(supportedRevitFileInfo)))

        supportedRevitFileList = list(supportedRevitFileInfo for supportedRevitFileInfo in supportedRevitFileList if (supportedRevitFileInfo.IsCloudModel() or RevitFileExists(supportedRevitFileInfo)))

        unsupportedRevitFileList = list(supportedRevitFileInfo for supportedRevitFileInfo in supportedRevitFileList if not HasAllowedRevitVersion(batchRvtConfig, supportedRevitFileInfo))

        unsupportedRevitFilePathRevitFileList = list(supportedRevitFileInfo for supportedRevitFileInfo in supportedRevitFileList if (not supportedRevitFileInfo.IsCloudModel() and not HasSupportedRevitFilePath(supportedRevitFileInfo)))

        supportedRevitFileList = list(supportedRevitFileInfo for supportedRevitFileInfo in supportedRevitFileList if ((supportedRevitFileInfo.IsCloudModel() or RevitFileExists(supportedRevitFileInfo)) and (HasAllowedRevitVersion(batchRvtConfig, supportedRevitFileInfo)) and (supportedRevitFileInfo.IsCloudModel() or HasSupportedRevitFilePath(supportedRevitFileInfo)))).OrderBy(
                lambda supportedRevitFileInfo: GetRevitFileSize(supportedRevitFileInfo) if not supportedRevitFileInfo.IsCloudModel() else System.Int64(0)  # dummy file size value for Cloud models
                ).ToList()
...
def RunSingleRevitTask(batchRvtConfig):
    ...
    if not aborted:
        ...
        batch_rvt_monitor_util.RunScriptedRevitSession(revitVersion, batchRvtScriptsFolderPath, batchRvtConfig.ScriptFilePath, [scriptData], 1, batchRvtConfig.ProcessingTimeOutInMinutes, batchRvtConfig.ShowRevitProcessErrorMessages, batchRvtConfig.TestModeFolderPath, Output)
    ...
    for revitVersion, supportedRevitFiles in GroupByRevitVersion(batchRvtConfig, supportedRevitFileList):
        ...
        while queuedRevitFiles.Any():
            ...
            if len(sessionRevitFiles) == 1:
                Output()
                Output("Processing Revit file (" + str(progressNumber) + " of " + str(totalFilesCount) + ")" + " in Revit " + RevitVersion.GetRevitVersionText(revitVersion) + " session.")
            else:
                Output()
                Output("Processing Revit files (" + str(progressNumber) + " to " + str(progressNumber + sessionFilesCount - 1) + " of " + str(totalFilesCount) + ")" + " in Revit " + RevitVersion.GetRevitVersionText(revitVersion) + " session.")
            ...
            for index, supportedRevitFileInfo in enumerate(sessionRevitFiles):
                ...
                if batchRvtConfig.EnableDataExport:
                    try:
                        snapshotDataExportFolderPath = snapshot_data_util.GetSnapshotFolderPath(batchRvtConfig.DataExportFolderPath, revitFilePath, isCloudModel, cloudProjectId, cloudModelId, batchRvtConfig.SessionStartTime)
            ...
            while scriptDatas.Any():
                ...
                if nextProgressNumber is None:
                    Output()
                    Output("WARNING: The Revit session failed to initialize properly! No Revit files were processed in this session!")  # Leave progress number as-is (i.e. do not skip any files if the Revit session terminated before processing any files.)
                else:
                    progressNumber = nextProgressNumber

                scriptDatas = (scriptDatas.Where(lambda scriptData: scriptData.ProgressNumber.GetValue() >= progressNumber).ToList())
...
def RunBatchRevitTasks(batchRvtConfig):
    ...
    if not aborted:
        ...
        try:
            if batchRvtConfig.EnableDataExport:
                session_data_exporter.ExportSessionFilesData(batchRvtConfig.SessionDataFolderPath, batchRvtConfig.SessionId, [supportedRevitFileInfo.GetRevitFileInfo().GetFullPath() for supportedRevitFileInfo in supportedRevitFileList])
        except Exception as ex:
            Output("Failed to write session data: " + str(ex))
...
def Main():
    ...
    if batchRvtConfig is None:
        ...
        if batchRvtConfig.EnableDataExport:
            ...
            session_data_exporter.ExportSessionData(batchRvtConfig.SessionId, batchRvtConfig.SessionStartTime, None, batchRvtConfig.SessionDataFolderPath, None)
        ...
        try:
            ...
            if batchRvtConfig.EnableDataExport:
                ...
                session_data_exporter.ExportSessionData(batchRvtConfig.SessionId, batchRvtConfig.SessionStartTime, sessionEndTime, batchRvtConfig.SessionDataFolderPath, sessionError)
