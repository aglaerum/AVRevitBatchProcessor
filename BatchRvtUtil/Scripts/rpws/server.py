...
class RevitServer(object):
    ...
    def _httpmethod(self, http_method, command, node_uri=None, rootcmd=False):
        ...
        if r.status_code == 200 and r.text:
            ...
            raise rpws.FilenFinnesAllerede(node_uri)
    ...
    def getmodelinfo(self, nodepath):
        # type: (str) -> rpws.models.ModelInfoEx
    ...
    def walk_asplan(self, top=None, topdown=True, digmodels=False):
        if not top:
            top = self.path
        entry_info = self.scandir(top)
        if topdown:
            # Yield before recursion if going top down
            print top,
            for inf in entry_info.folders:
                print inf.name, "---- " + inf.path
            yield top, entry_info.folders, entry_info.files, entry_info.models
        # Recurse into sub-directories
        for finfo in entry_info.folders:
            for x in self.walk_asplan(top=finfo.path, topdown=topdown):
                try:
                    print x.path
                    yield x
                except:
                    print "Fail paa en modell"
                    pass
"""
        # Recurse into sub-directories inside models
        ...
        if digmodels:
            for minfo in entry_info.models:
                for x in self.walk(top=minfo.path, topdown=topdown, digmodels=digmodels):
                    yield x
        if not topdown:
            # Yield before recursion if going top down
            yield top, entry_info.folders, entry_info.files, entry_info.models
